...
my %wrong_base = (
    2  =>  ["2.1", "0.9", "01.23", "01010.1019"],
    8  =>  ["8.", "1234.84321", ".0a12"],
   16  =>  ["g.3", "12345.defg0234", "g.hi"]
);
...
#
$test {  2  } -> no_match ("0B101.01", reason => "Prefix has incorrect base");
$test { "2^"} -> no_match ("0b101.01", reason => "Prefix has incorrect base");
$test { 16  } -> no_match ("0X123.45", reason => "Prefix has incorrect base");
$test {"16^"} -> no_match ("0x123.45", reason => "Prefix has incorrect base");
...
foreach my $key (qw [2 2^ 8 8^]) {
    $test {$key} -> no_match ("0x1.1", reason => "Incorrect prefix");
    $test {$key} -> no_match ("0X1.1", reason => "Incorrect prefix");
}
foreach my $key (qw [8 8^]) {
    $test {$key} -> no_match ("0b12.34", reason => "Incorrect prefix");
    $test {$key} -> no_match ("0B12.34", reason => "Incorrect prefix");
}
...
#
$test { 16  } -> no_match ("100.A78", reason => "Incorrect casing");
$test {"16^"} -> no_match ("100a.78", reason => "Incorrect casing");
